...
@Log4j2
@CommandLine.Command(name = "railsim-scenario", mixinStandardHelpOptions = true, subcommands = {RunRailsimScenario.RunCommand.class, RunRailsimScenario.ReconstructCommand.class})
public class RunRailsimScenario {
    ...
    static class RunCommand implements Callable<Integer> {
        ...
        @CommandLine.Option(names = {"--cleanup"}, defaultValue = "false", description = "Delete run outputs after analysis to save space.")
    ...
    static class ReconstructCommand implements Callable<Integer> {
        ...
        @CommandLine.Option(names = {"-r", "--runs"}, required = true, split = ",", description = "Specific Run IDs to re-run.")
        private Set<String> runIds;

        @CommandLine.Option(names = {"-w", "--worker-threads"}, defaultValue = "-1", description = "The number of worker threads in the simulation executor (default is number of cores).")
        private int workerThreads;
